...
PH 264 is fitting different kinds of models to data, and extracting parameters from them. An important example is exponential
decay, which shows up in radioactivity, capacitors discharging, and it can also happen in astrophysics.

The physical model for exponential decay is: N(t)=N_0 e^(-lambdat), where N(t) is the quantity (Edit: of what?) at time t, N_0 is the intial 
quantity, and lambda is the decay constant. We are not given lamba, so we have to find it through experiment and using data. 
