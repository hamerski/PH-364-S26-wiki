...
For example, if you suspect that your experiment can be modeled by an equation such as A*sin(phi), where A and phi are variables, and you wanted to see how close your model was to some measured phenomena, you would use the Chi Squared calculation in a nested for loop for both A and phi. Then, find the minimum chi-squared and plot a graph to see how dependent each variable is. By calculating the reduced chi-squared, a numerical value can be found that represents how close the minimum chi-squared is to the real values.
